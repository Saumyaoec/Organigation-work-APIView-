...
class JobUserProfileUpdateAPIView(APIView):
	...
	def put(self, request, slug, format=None):
		...
		serializer = JobUsersProfileSerializer(job_user_profile, data=request.data)
		if serializer.is_valid():
			serializer.save()
			return Response(serializer.data, status=status.HTTP_200_OK)
		return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class JobUpdateAPIView(APIView):
	...
	def put(self, request, slug, format=None):
		JobDetails = self.get_object(slug)
		serializer = JobDetailsSerializer(JobDetails, data=request.data, context={'request': request})
		if serializer.is_valid():
			serializer.save()
			return Response(serializer.data, status=status.HTTP_200_OK)
		return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
	...
	def post(self, request, *args, **kwargs):
		...
		if serializer.is_valid():
			if AppliedJobs.objects.filter(user=request.user, job=request.data.get('job')).exists():
				...
				users_profile = JobUsersProfile.objects.filter(user=request.user).first()
				applied_job = serializer.save(user=request.user, users_profile=users_profile)
...
class ZoomMeetingsUpdateAPIView(APIView):
	...
	def put(self, request, slug, format=None):
		skill = self.get_object(slug)
		serializer = ZoomMeetingsSerializer(skill, data=request.data, context={'request': request})
		if serializer.is_valid():
			serializer.save()
			return Response(serializer.data, status=status.HTTP_200_OK)
		return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class ZoomMeetingsUsersUpdateAPIView(APIView):
	...
	def put(self, request, slug, format=None):
		skill = self.get_object(slug)
		serializer = ZoomMeetingsUsersSerializer(skill, data=request.data, context={'request': request})
		if serializer.is_valid():
			serializer.save()
			return Response(serializer.data, status=status.HTTP_200_OK)
		return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class ZoomLinkAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        response = client.meeting.create(user_id='me', topic='Interview Meeting')
...
class StatusChangeAPIView(APIView):
	...
	def put(self, request, slug, format=None):
		applied_jobs = self.get_object(slug)
		serializer = StatusChangeSerializer(applied_jobs, data=request.data, context={'request': request}, partial=True)
		if serializer.is_valid():
			if request.user == applied_jobs.job.job_creator.emp_user:
				serializer.save(status=request.data.get('status', applied_jobs.status))
				return Response(serializer.data, status=status.HTTP_200_OK)
			else:
...
class ZoomLinkIndividualAPIView(APIView):
    ...
    def post(self, request, slug):
        ...
        if serializer.is_valid():
            ...
            response = client.meeting.create(user_id='me', topic=data['meeting_topic'], start_time=start_time)
            meeting_info = response.json()
            join_url = meeting_info.get('join_url')
