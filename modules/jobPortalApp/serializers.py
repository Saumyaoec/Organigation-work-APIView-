...
class JobUsersProfileSerializer(serializers.ModelSerializer):
	class Meta:
		...
		fields = ['slug', 'resume', 'higer_secondary_year', 'secondary_year', 'grad_year', 'post_grad_year', 'looking_for']

	def validate_higer_secondary_year(self, value):
		if not (2000 <= value <= 2023):
			raise serializers.ValidationError("Higher secondary year must be year between 2000-2023.")
		return value

	def validate_secondary_year(self, value):
		if not (2000 <= value <= 2023):
			raise serializers.ValidationError("Secondary year must be year between 2000-2023.")
		return value

	def validate_grad_year(self, value):
		if not (2000 <= value <= 2023):
			raise serializers.ValidationError("Graduation year must be year between 2000-2023.")
		return value

	def validate_post_grad_year(self, value):
		if not (2000 <= value <= 2023):
			raise serializers.ValidationError("Post graduation year must be year between 2000-2023.")
		return value
...
class AddressDetailsSerializer(serializers.ModelSerializer):
	...
	pincode = serializers.CharField(allow_null=True, allow_blank=True, required=False)
...
class JobDetailsSerializer(serializers.ModelSerializer):
	chose_default_address = serializers.PrimaryKeyRelatedField(queryset=AddressDetails.objects.all(), allow_null=True)
	address = AddressDetailsSerializer()
	class Meta:
		model = JobDetails
		fields = ('gid', 'slug',  'chose_default_address', 'company', 'job_title', 'job_description', 'experiance', 'salary', 'address', 'hiring_members', 'industry_type', 'employement_type', 'qualification')

	def create(self, validated_data):
		address_data = validated_data.pop('address')
		chose_default_address = validated_data.pop('chose_default_address', None)
		if chose_default_address is None:
			address = AddressDetails.objects.create(**address_data)
		else:
			address = chose_default_address
		job_details = JobDetails.objects.create(address=address, **validated_data)
	...
	def validate_hiring_members(self, value):
		...
		"""
		if value <= 0:
			raise serializers.ValidationError("hiring_members must be greater than 0.")
		return value
...
class AppliedJobsSerializer(serializers.ModelSerializer):
	...
	class Meta:
		...
		read_only_fields = ('gid', 'user', 'users_profile', 'slug', 'date_posted',)
...
class ZoomMeetingsSerializer(serializers.ModelSerializer):
	class Meta:
		model = ZoomMeetings
		fields = ( 'meeting_topic', 'meeting_date', 'meeting_time', 'meeting_duration',)

class ZoomMeetingsUsersSerializer(serializers.ModelSerializer):
	class Meta:
		model = ZoomMeetingsUsers
		fields = ('slug', 'zoom_meeting', 'status', 'mail_send', 'message', 'jobs')
